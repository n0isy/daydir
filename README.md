...
```text
dist/daydir.exe
dist/daydir.exe.sha256
build/daydir.lst
build/daydir.map
```

`dist/` and `build/` are generated and ignored by Git. To choose another output
...
The checked build uses NASM 2.16.03 and is byte-identical to the executable confirmed
working by the user:

```text
Size:    984 bytes
SHA-256: 9dd5b92637e3a22ebe754e214144f914c8b9ff8ea45e0f4b5ec52ba1cffea694
```
...
| [build.py](build.py) | Reproducible assembly, listing, map and checksum |
| [inspect_pe.py](inspect_pe.py) | Complete byte accounting and hex report |
| [tests/](tests/) | PE structure, filesystem and calendar checks |
...
Regenerate the report after building:

```text
python inspect_pe.py
```

The script uses NDISASM, included with NASM, and Python's standard library. It also
creates `build/bytes.csv`, with one row per byte. `NDISASM` can override the
disassembler executable path.

## Local checks

```text
python tests/check_pe.py
...
Local result: **40 PE launches**, PE structure checks and the full calendar cycle passed.
...
for pushes to `main`, version tags (`v*`), pull requests and manual dispatch. It installs NASM, assembles the EXE,
checks its PE structure, regenerates this binary's analysis, and uploads the EXE,
checksum and documentation as the `daydir-windows-x64` artifact.

For a version tag, a dependent release job downloads the verified artifact, checks
the executable's SHA-256 checksum and publishes a GitHub Release with the EXE,
checksum and documentation. For example:
...
GitHub Actions checks assembly, PE structure and the generated byte report.
The local Windows/Wine execution results are documented separately above.
